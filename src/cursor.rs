...
//! `ptr::write` — never `&`/`&mut` over caller memory, which may be uninitialized
//! (`fh_cursor` on entry) or aliased (`ip == sp`).
//!
//! A cursor carries a per-claim **nonce** checked against the slot's claim sequence, so
//! a stale cursor (copied struct, recycled slot) no-ops instead of corrupting the slot's
//! next owner.
//!
...
//!  2. Recovering from a faulting read by `longjmp`ing out of `fh_step` (Julia's
//!     `jl_set_safe_restore`) crosses this library's Rust frames — defined only while
//!     they hold no pending destructors (RFC 2945). The read path is kept
//!     destructor-free, but exact bounds are the supported configuration; the longjmp
//!     path is best-effort.
...
pub struct FhCursor {
    magic: u64,
    slot: u64,
    /// Claim nonce from [`state::claim_slot`]; must still match the slot's `seq` for
    /// this cursor to count as the live owner.
    nonce: u64,
...
/// Pointers are non-null (FFI layer checks) and correctly sized; `*cur` may be
/// uninitialized (it is only written).
...
pub fn cursor_init_bounds(
    ...
    // Write-only teardown: *cur may be uninitialized C memory, so never read it — which
    // also means a still-live cursor's slot is NOT released here (fini before re-init).
    // SAFETY: cur is valid for writes.
    unsafe { cur.write(FhCursor::dead()) };
    // SAFETY: every capture/extraction path writes all five context words.
...
/// error. Async-signal-safe. Outputs use raw writes, so `out_ip == out_sp` aliasing is
/// last-write-wins rather than UB.
pub fn step(cur: *mut FhCursor, out_ip: *mut u64, out_sp: *mut u64) -> c_int {
    ...
    // SAFETY: non-null; copied out so output writes cannot alias our view.
    ...
    // SAFETY: live owner (nonce checked) until fini; single `&mut`, split into disjoint
    // field borrows below.
    let inner = unsafe { slot.inner_mut() };
    if inner.done {
        return 0;
    ...
    match result {
        ...
        // Any unwind error (incl. a bad stack read) is a normal truncation: the frames
        // emitted so far are valid.
        Err(_) => {
            inner.done = true;
            0
        }
...
/// cursor). Read-only: takes only a shared view of the slot state, so it never
/// manufactures a `&mut` from a `const fh_cursor *`.
pub fn get_reg(cur: *const FhCursor, out_ip: *mut u64, out_sp: *mut u64) {
    ...
    // SAFETY: non-null; copy out (see `step`).
    let c: FhCursor = unsafe { cur.read() };
    if !c.is_live() {
        return;
    }
    if let Some(slot) = state::slot(c.slot as usize) {
        // SAFETY: owned by this cursor; `step`'s exclusive borrow ends before it returns.
...
pub fn cursor_fini(cur: *mut FhCursor) {
    ...
    if c.magic == CURSOR_MAGIC && c.slot != u64::MAX {
        // The nonce CAS inside makes a stale copy's release a no-op.
